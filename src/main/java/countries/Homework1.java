...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().filter(Predicate.not(country -> country.getName().contains("Island"))).findAny().isPresent();
    }
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream().filter(country -> country.getName().toUpperCase().contains("ISLAND")).map(country -> country.getName()).findFirst();
    }
    ...
    public void streamPipeline3() {
        countries.stream().filter(country -> country.getName().toUpperCase().charAt(0)==country.getName().toUpperCase().charAt(country.getName().length()-1)).map(country -> country.getName()).forEach(System.out::println);
    }
    ...
    public void streamPipeline4() {
        countries.stream().sorted(Comparator.comparingLong(Country::getPopulation)).limit(10).mapToLong(Country::getPopulation).forEach(System.out::println);
    }
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream().filter(Predicate.not(country -> country.getTranslations().containsKey("es"))).count();
    }
    ...
    public void streamPipeline11() {
        countries.stream().flatMap(country -> country.getTranslations().values().stream()).sorted().forEach(System.out::println);
    }
    ...
    public double streamPipeline12() {
        return countries.stream().mapToDouble(country -> country.getName().length()).average().orElse(-1);
    }
    ...
    public void streamPipeline13() {
        countries.stream().filter(country -> country.getArea()==null).map(country -> country.getRegion()).forEach(System.out::println);
    }
    ...
    public Optional<Country> streamPipeline14() {
        ...
        return countries.stream().filter(Predicate.not(country -> country.getArea()==null)).sorted(Comparator.comparing(Country::getArea).reversed()).findFirst();
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea()!=null).filter(country -> country.getArea().compareTo(BigDecimal.ONE)==-1).map(country -> country.getName()).forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
        countries.stream().filter(country -> country.getRegion().equals(Region.EUROPE) || country.getRegion().equals(Region.ASIA)).flatMap(country -> country.getTimezones().stream()).distinct().forEach(System.out::println);
    }
